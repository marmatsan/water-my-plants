...
Version values belong in `build-logic/versions.properties` and dependency tree
definitions belong in `build-logic/dependencies`.
...
```powershell
.\gradlew.bat -p build-logic :android:check
```
