...
class DocumentationValidator {
    ...
    private fun validateMetadata(
        ...
        if (actualType != expectedType) {
            errors.add(
                element =
                    "[$path] Frontmatter type '${actualType.orEmpty()}' " +
                        "does not match path type '$expectedType'.",
            )
        }
        ...
        val reviewDate =
            frontmatter.metadata["last-reviewed"]?.let { value ->
                try {
                    LocalDate.parse(
                        value,
