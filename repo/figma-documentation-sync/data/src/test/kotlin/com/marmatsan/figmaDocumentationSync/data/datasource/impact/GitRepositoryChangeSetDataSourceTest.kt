...
internal class GitRepositoryChangeSetDataSourceTest : FunSpec({
    test("read returns repository paths changed from origin main") {
        ...
        try {
            git(repository, "init")
            git(repository, "config", "user.name", "Figma impact test")
            git(repository, "config", "user.email", "figma-impact@example.invalid")
            File(repository, "base.txt").writeText("base")
            git(repository, "add", ".")
            git(repository, "commit", "-m", "test: base")
            git(repository, "branch", "-M", "main")
            git(repository, "remote", "add", "origin", repository.absolutePath)
            git(repository, "fetch", "origin", "main:refs/remotes/origin/main")
            val baseSha = git(repository, "rev-parse", "HEAD")

            File(repository, "changed.txt").writeText("changed")
            git(repository, "add", ".")
            git(repository, "commit", "-m", "test: change")
