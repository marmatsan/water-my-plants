...
test("included build settings catalog targets are declared catalog targets", () => {
  const targets = CATALOG_TREE_TARGETS.filter((candidate) =>
    candidate.name.startsWith("gradlePlugins.") ||
      candidate.name.startsWith("figmaDocumentationSync.")
  );
  ...
  assert.deepEqual(
    ...
    [
      ...
      ["figmaDocumentationSync.libraries", "declaredCatalogTarget"],
      ["figmaDocumentationSync.plugins", "declaredCatalogTarget"],
    ]
  );
});
