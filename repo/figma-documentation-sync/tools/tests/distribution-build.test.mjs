...
const projectConfig = process.env.FIGMA_DOCUMENTATION_SYNC_PROJECT_CONFIG;

test("materializes a writer from the Kotlin-generated JSON project config", async () => {
  assert.ok(projectConfig, "FIGMA_DOCUMENTATION_SYNC_PROJECT_CONFIG is required.");
  const outputRoot = await mkdtemp(join(tmpdir(), "figma-documentation-sync-tools-"));
  try {
    const childEnvironment = { ...process.env };
    delete childEnvironment.FIGMA_DOCUMENTATION_SYNC_PROJECT_CONFIG;
    ...
    assert.doesNotMatch(generatedWriter, /@figma-documentation-sync\/project-config/);
    ...
    assert.doesNotMatch(preview, /@figma-documentation-sync\/project-config/);
  } finally {
