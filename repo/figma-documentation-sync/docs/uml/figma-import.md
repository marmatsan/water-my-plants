...
```text
tmp/uml/figmaDocumentationSync/
```
...
```powershell
plantuml -tsvg `
  -o "$PWD\tmp\uml\figmaDocumentationSync" `
  repo\figma-documentation-sync\docs\uml\diagrams\figma-design-model-feature.puml

repo\figma-documentation-sync\docs\uml\tools\sanitize-svg-for-figma.ps1 `
  -InputPath tmp\uml\figmaDocumentationSync\figma-design-model-feature.svg `
  -OutputPath tmp\uml\figmaDocumentationSync\figma-design-model-feature.figma.svg
```
...
for example `figmaDocumentationSync` for module diagrams or `projectModuleDependencies`
...
```powershell
inkscape tmp\uml\figmaDocumentationSync\diagram.sanitized.svg `
  --export-type=svg `
  --export-filename=tmp\uml\figmaDocumentationSync\diagram.outlined.svg `
  --export-text-to-path `
  --export-plain-svg
```
