...
- Treat direct `.artifact` and `.artifacts bundle` children as interchangeable
  ...
  after the replacement has been created.
...
structure has enough interchangeable slots is reshaped in place. A node whose
total slot capacity has become too small is not updated partially: its path is
reassigned to a compatible replacement and the old node is removed as stale in
the same visual transaction.
