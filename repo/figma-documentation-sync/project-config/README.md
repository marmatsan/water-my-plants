# Figma Documentation Sync Project Configuration

`project-config` contains repository adapters for the portable
`figma-documentation-sync` engine. The reusable contract, Kotlin implementation,
...
- `src/main/kotlin/.../WaterMyPlantsFigmaDocumentationSyncGradlePlugin.kt`, which
...
```kotlin
plugins {
    id("com.marmatsan.waterMyPlantsFigmaDocumentationSync")
}
```
...
2. applies and configures `com.marmatsan.figmaDocumentationSync`;
...
`build/generated/figma-documentation-sync/writer-project-config.json`; the official
...
See the [adoption guide](../docs/guides/adopting-figma-documentation-sync.md) for that
