...
internal class EnvironmentTeamCityAutomationCredentialsProviderTest :
    FunSpec(
        {
                ...
                val provider =
                    EnvironmentTeamCityAutomationCredentialsProvider(
                        ...
                        cloudflareAccessTokenProvider =
                            CloudflareAccessTokenProvider { _, _, _, _ ->
                                error("Cloudflare exchange must not run")
                            },
                    )

                provider.load("https://teamcity.example/") shouldBe
            ...
            test("exchanges service credentials when no short-lived token exists") {
                ...
                val provider =
                    EnvironmentTeamCityAutomationCredentialsProvider(
                        environment = values::get,
                        cloudflareAccessTokenProvider =
                            CloudflareAccessTokenProvider { url, token, id, secret ->
                                exchangeArguments =
                                    listOf(
                                        url,
                                        token,
