...
1. Ports in `com.marmatsan.figmaDocumentationSync.domain.port` describe what the
   generator needs.
2. Models in `com.marmatsan.figmaDocumentationSync.domain.model` describe the stable
   data contract.
3. Samples in `com.marmatsan.figmaDocumentationSync.domain.samples` show small
...
# Package com.marmatsan.figmaDocumentationSync.domain.model.modules
...
Ports keep the domain independent from IO. Adapters in `figmaDocumentationSync-data`
