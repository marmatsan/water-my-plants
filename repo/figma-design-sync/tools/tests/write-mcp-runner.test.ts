...
test("official runner defaults to the complete visual sync without metadata", async () => {
  ...
  try {
    ...
    assert.match(manifest.payloadImage.sha256, /^sha256:[a-f0-9]{64}$/);
...
test("official runner supports an explicitly partial diagnostic target", async () => {
  ...
  try {
    ...
    assert.match(stageSource, /setSharedPluginData\(namespace, "writerHash", expected\.writerHash\)/);
    ...
    assert.match(runTargetSource, /getSharedPluginData\(namespace, "writerHash"\)/);
